<template>
	<div
		class="absolute inset-0 ml-divider h-screen z-50 flex items-center justify-center transition"
	>
		<div class="w-5/12 h-full bg-white flex flex-col relative shadow-2xl z-20">
			<div class="bg-darkDivider px-8 pt-8 w-full">
					...
					<div class="flex justify-between w-full">
						...
						>
							...
							>
								...
								>
								<span
									class="text-red-600 font-semibold border-t px-4 py-1 text-sm hover:bg-gray-100 rounded"
									>Удалить</span
								>
							</div>
						</button>
				...
				<div class="w-full flex flex-col mb-5">
					...
					>
						...
						<div class="absolute bottom-0 left-0 w-full h-1 bg-gray-900 rounded-xl flex z-10">
							<div
								class="w-1/4 bg-blue-500 rounded-l-xl"
				...
				<div class="flex flex-nowrap space-x-3">
					<span
						...
						:class="idx === selectableTab ? 'text-white border-b-4' : 'text-gray-600'"
			...
			<div class="flex-grow">
				<HouseTab ref="houseTab" v-if="selectableTab === 0" :client="client" @openSave="openSave" />
			...
			<transition name="slideUp">
				<div
					...
					class="flex-shrink-0 w-full h-divider border-t flex items-center px-8 absolute bottom-0"
				>
					<button
					...
					<button class="text-gray-500 font-semibold focus:outline-none ml-4" @click="cancel()">
						Отменить
					</button>
				</div>
			</transition>
...
export default {
	...
	methods: {
		async saveEdit() {
			...
			info.status = this.categories.find((category) => category.id === this.categoriesId).title;
