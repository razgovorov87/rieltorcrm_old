...
export default {
    ...
    actions: {
        ...
        async addNewClient({dispatch, commit}, {phone, name, surname, company, budget, city, adress}) {
            const fio = name && surname ? name + ' ' + surname : ''
            await firebase.database().ref(`/clients/`).push({
                phone,
                fio,
                company,
                budget,
        ...
        async fetchClients({dispatch, commit}) {
            const response = (await firebase.database().ref('/clients').once('value')).val()
        ...
        async catchNewClient({dispatch, commit}) {
            const uid = await dispatch('getUid')
            let arr = (await firebase.database().ref('/clients').once('value')).val()
            arr = Object.keys(arr).map(key => ({...arr[key], id: key,}))
            let item = arr.find(item => !item.agent)
            if(!item) return 'empty_list'
            await firebase.database().ref(`/clients/${item.id}`).update({
                agent: uid,
                logs: {}
            })
            ...
            item = (await firebase.database().ref(`/clients/${item.id}`).once('value')).val()
